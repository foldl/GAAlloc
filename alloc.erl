...
start(Fn) ->
    ...
    F = fun () -> 
            {PidDict, _} = lists:foldl(fun ({Inf, Sup, FirstGen, AllocProg, ASchedule}, {DictAcc, Counter}) ->
                ...
                unconsult(Path ++ integer_to_list(Counter) ++ ".sch", {Inf, Sup, ASchedule, VarGroup}),
            ...
            unconsult(Path ++ "plan", dict:to_list(PidDict)),
            ga_supervisor(PidDict)
    end,
    spawn(F).
    ...
    receive 
        ...
        {quit, ID} -> 
            ...
            case dict:size(PidDict10) of
                0 -> error_logger:info_report("All workers exit.");
                _ -> ga_supervisor(PidDict10)
            end;
        Msg -> 
...
save_population(Path, ID, Population, Type) ->
    ...
    {ok, Fid} = file:open(Path ++ Fn ++ "." ++ Type, [write]),
...
unconsult_prog(Path, {Inputs, Outputs, #prog{aliasSets = AliasSets, aliasDict = AliasDict} = _Prog}) ->
    {ok, Fid} = file:open(Path ++ "prog", [write]),
...
population_cleanup(NextGen) ->
    sets:to_list(sets:from_list(NextGen)).
